...
### 11. AI COLOR PALETTE & PROMPT GENERATOR

**Main Feature**: A modern web app for generating color palettes and AI-ready design prompts.

#### Server Module (`/user-app/src/server/palette/`)
- **db.ts**: Store for saved palettes with colors, style, mood, userId
- **index.ts**: Module with queries (getSavedPalettes) and mutations (savePalette, deletePalette)

#### HomePage Features (`/user-app/src/client/pages/HomePage.tsx`)
1. **Color Palette Generator**
   - 10 brand-inspired styles: Nothing, Claude, Vercel, Linear, Stripe, Notion, Spotify, Discord, Figma, Glass
   - Separate Dark/Light mode toggle (independent from style)
   - 6 colors per palette: primary, secondary, accent, background, surface, text
   - Auto-generated palette names (e.g., "Midnight Glow", "Aurora Breeze")
   - Click-to-copy hex codes with toast notifications
   - Nothing (transparent tech) style as default

2. **Live UI Preview**
   - Compact mini preview showing navbar, cards, buttons, stats
   - Dynamic styling based on selected palette and mode

3. **Vibe Mode (Simplified)**
   - 4 simple vibes: minimal, bold, playful, techy
   - Generates concise AI prompts with color info and style guidance

4. **Dev Export**
   - CSS Variables, Tailwind config, JSON formats
   - One-click copy to clipboard

5. **Saved Palettes**
   - Requires authentication to save
   - Load/delete saved palettes
   - Stored in MongoDB via Modelence Store

#### UI Notes
- Compact 3-column responsive layout (less scrolling needed)
- Dark UI theme with glass-style cards
...
- AI Color Palette & Prompt Generator as the main feature
...
- Authentication system in place (optional for saving palettes)
